...
module.exports = async (client, guild) => {
  ...
  const guild_settings_pos = client.guild_settings
    .map((e) => e.guildId)
    .indexOf(guild.id);
  const setttings_db =
    await client.guild_settings[guild_settings_pos].settings_db.getData("/");
  ...
  if (setttings_db.nameday_channel) {
    const index = client.nameday_channels.indexOf(
      setttings_db.nameday_channel.nameday_channel_id,
    );
    if (index > -1) {
