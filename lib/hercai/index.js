 ...
 * @param {string} apiKey
 * @example question({model:"v3-beta",content:"how are you?"})
 * @type {string} The Question You Want to Ask Artificial Intelligence.
 * @returns {Promise<object>}
 * @async
 */
export async function question({ model = "v3", content, personality = "", apiKey = "" }) {
  if (
    ![
 ...
 * @param {string} apiKey
 * @example drawImage({model:"v3",prompt:"anime girl"})
 * @type {string} Tell Artificial Intelligence What You Want to Draw.
 * @returns {Promise<object>}
 * @async
 */
export async function drawImage({ model = "v3", prompt, negative_prompt = "", apiKey = "" }) {
  if (
    ![
      "v1", "v2", "v2-beta", "v3",
 ...
 * @param {string} apiKey
 * @example betaQuestion({content:"how are you?"})
 * @type {string} The Question You Want to Ask Artificial Intelligence.
 * @returns {Promise<object>}
 * @async
 */
export async function betaQuestion({ content, personality = "", user = "", apiKey = "" }) {
  if (!content) throw new Error("Please specify a question!");
 ...
 * @param {string} apiKey
 * @example betaDrawImage({prompt:"anime girl"})
 * @type {string} Tell Artificial Intelligence What You Want to Draw.
 * @returns {Promise<object>}
 * @async
 */
export async function betaDrawImage({
  prompt,
  negative_prompt = "",
  sampler = "DPM-Solver",
  image_style = "Null",
