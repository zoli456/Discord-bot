...
class TempChannelsManager extends VoiceChannelsManager_1.VoiceChannelsManager {
  ...
  listenToChannelEvents() {
    ...
    this.client.on(discord_js_1.Events.ChannelDelete, (channel) =>
      this.cleanUpAfterDeletion.apply(this, [
        channel,
      ]),
    );
    this.on(
      ...
      async (parent) =>
        await this.restoreAfterCrash.apply(this, [
          parent,
        ]),
    );
    this.on(
  ...
  async restoreAfterCrash(parentData) {
    ...
    const children = (category?.children ?? voiceChannel.guild.channels).cache.filter(
      (c) =>
        ...
        c.permissionOverwrites.cache.some((po) => po.type === discord_js_1.OverwriteType.Member),
    );
    for (let child of [
      ...children.values(),
    ]) {
  ...
  cleanUpAfterDeletion(channel) {
    if (!channel || channel.type !== discord_js_1.ChannelType.GuildVoice) return;
