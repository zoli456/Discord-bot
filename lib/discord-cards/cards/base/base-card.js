...
import rgbToHex from "../../utils/rgb-hex.js";

/**
 * Base Card Builder for creating customizable cards
 */
export class BaseCardBuilder {
    ...
    if (!options?.only || options.only.includes("background")) {
      ...
      if (this.backgroundImgURL) {
        ...
        if (this.backgroundColor.waves) {
          ...
          if (wavesColor.includes("rgb")) wavesColor = (0, rgbToHex)(wavesColor);
          ctx.beginPath();
          ctx.fillStyle = (0, hexToRgbA)(wavesColor, 1);
          ctx.moveTo(0, 120);
          ...
          ctx.closePath();
          ctx.beginPath();
          ctx.fillStyle = (0, hexToRgbA)(wavesColor, 0.74);
          ctx.moveTo(0, 176.82);
          ...
          ctx.closePath();
          ctx.beginPath();
          ctx.fillStyle = (0, hexToRgbA)(wavesColor, 0.15);
          ctx.moveTo(0, 290.92);
   ...
   */
  async build(options) {
    const canvas = (0, createCanvas)(800, 350);
