...
class RankCardBuilder {
  ...
  async draw(ctx, canvasWidth, canvasHeight, options) {
    if (!options?.only || options.only.includes("background")) {
      ...
      if (this.backgroundImgURL) {
        try {
          ...
          throw new Error(
            "Error loading the background image. The URL may be invalid.",
          );
        }
      } else {
        ...
        if (this.backgroundColor.bubbles) {
          ...
          ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(
            this.backgroundColor.bubbles,
            0.31,
          );
          ctx.fill();
          ctx.closePath();
    ...
    if (
      ...
      if (this.avatarImgURL) {
        ...
        try {
          ...
          throw new Error(
            "Error loading the avatar image. The URL may be invalid.",
          );
        }
        ctx.restore();
        // Status
    ...
    if (!options?.only || options.only.includes("progressBar")) {
      ...
      ctx.closePath();
      // Progress Bar Front
      const currentPercentXP = Math.floor(
        (this.currentXP / this.requiredXP) * 100,
      );
    ...
    if (!options?.only || options.only.includes("nickname")) {
      // Nickname
      const nicknameFont = this.nicknameText.font
        ? this.nicknameText.font
        : this.fontDefault;
    ...
    if (!options?.only || options.only.includes("rank")) {
      ctx.textAlign = "right";
      const rankFont =
        this.rankPrefix && this.rankPrefix.font
          ? this.rankPrefix.font
          : this.fontDefault;
      const rankContent =
        this.rankPrefix && this.rankPrefix.content
          ? this.rankPrefix.content
          : "RANK";
      ctx.fillStyle =
    ...
    if (!options?.only || options.only.includes("lvl")) {
      // LVL
      const lvlFont =
        this.lvlPrefix && this.lvlPrefix.font
          ? this.lvlPrefix.font
          : this.fontDefault;
      const lvlContent =
        this.lvlPrefix && this.lvlPrefix.content
          ? this.lvlPrefix.content
          : "LVL";
      ctx.fillStyle =
