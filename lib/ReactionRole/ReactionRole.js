...
class ReactionRole extends discord_js_1.Client {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      for (const clickable of message.clickables) {
        ...
        if (clickable.onRemove)
          clickable.onRemove = (0, utils_1.stringifyFunction)(
            clickable.onRemove,
          );
      }
  ...
  async init(client) {
    ...
    for (const message_id in this.config) {
      ...
      for (const clickable of message.clickables) {
        if (
          ...
          !((_b = msg.reactions.cache.get(clickable.clickable_id)) === null ||
          _b === void 0
            ? void 0
            : _b.users.cache.has(
                (_c = this.user) === null || _c === void 0 ? void 0 : _c.id,
              ))
        )
    ...
    client.logger.log(
      `Activated reaction roles on ${Object.keys(this.config).length} messages.`,
    );
  }
  reInit() {
    return __awaiter(this, void 0, void 0, function* () {
  ...
  async reaction(client, reaction, user) {
    ...
    if (!(0, lodash_1.has)(this.config, reaction.message.id)) return;
    const message = (0, lodash_1.get)(this.config, reaction.message.id);
    const clickable = message.clickables.find(
      (clickable) =>
        clickable.clickable_id === (reaction.emoji.id || reaction.emoji.name),
    );
    if (!clickable) return;
    const member =
    ...
    const roles = clickable.roles.filter((id) =>
      clickable.type === types_1.EType.REMOVE
  ...
  async unreaction(client, reaction, user) {
    ...
    if (!clickable) return;
    if (clickable.type === types_1.EType.ONCE) return;
    const member =
