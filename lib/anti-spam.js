...
class AntiSpamClient extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.options = {
      ...
      kickThreshold: options.kickThreshold || 6,
      banThreshold: options.banThreshold || 6,

      maxInterval: options.maxInterval || 2000,
      maxDuplicatesInterval: options.maxDuplicatesInterval || 10_000,

      maxDuplicatesWarn: options.maxDuplicatesWarn || 3,
      maxDuplicatesMute: options.maxDuplicatesMute || 6,
      maxDuplicatesKick: options.maxDuplicatesKick || 6,
      maxDuplicatesBan: options.maxDuplicatesBan || 6,
      ...
      banErrorMessage:
        options.banErrorMessage || "Could not ban **{user_tag}** because of improper permissions.",
      muteErrorMessage:
      ...
      removeBotMessagesAfter: options.removeBotMessagesAfter || 20_000,
   ...
   * @returns boolean.
   */
  sendActionMessage(message, action, lang) {
    if (action === "warn") {
      message.channel
        .send(this.format(lang.warnMessage, message))
        .then((msg) => {
          if (this.options.removeBotMessages) {
            setTimeout(() => msg.delete(), this.options.removeBotMessagesAfter);
          }
        })
        .catch((e) => {
          if (this.options.verbose) {
            console.error(`DAntiSpam (warnUser#sendSuccessMessage)[341]: ${e.message}`);
          }
        });
      return true;
    } else if (action === "kick") {
      message.channel
        .send(this.format(lang.kickMessage, message))
        .then((msg) => {
          if (this.options.removeBotMessages) {
            setTimeout(() => msg.delete(), this.options.removeBotMessagesAfter);
          }
        ...
        .catch((e) => {
          if (this.options.verbose) {
            console.error(`DAntiSpam (kickUser#sendSuccessMessage)[352]: ${e.message}`);
          }
        });
      return true;
    } else if (action === "mute") {
      message.channel
        .send(this.format(lang.muteMessage, message))
        .then((msg) => {
          if (this.options.removeBotMessages) {
            setTimeout(() => msg.delete(), this.options.removeBotMessagesAfter);
          }
        ...
        .catch((e) => {
          if (this.options.verbose) {
            console.error(`DAntiSpam (muteUser#sendSuccessMessage)[363]: ${e.message}`);
          }
        });
      return true;
    } else if (action === "ban") {
      message.channel
        .send(this.format(lang.banMessage, message))
        .then((msg) => {
          if (this.options.removeBotMessages) {
            setTimeout(() => msg.delete(), this.options.removeBotMessagesAfter);
          }
        ...
        .catch((e) => {
          if (this.options.verbose) {
            console.error(`DAntiSpam (banUser#sendSuccessMessage)[374]: ${e.message}`);
          }
        });
    }
  }
    ...
    if (this.options.modLogsEnabled) {
      ...
      if (modLogChannel) {
        if (this.options.modLogsMode === "embed") {
          ...
          modLogChannel
            .send(`${msg.author}*(${msg.author.id})* has been **${action}** for **spam**.`)
            .catch((e) => {
              if (this.options.verbose) {
                console.error(
                  "DAntiSpam (log#noMessageSent): The mod log message could not be sent.",
        ...
        if (this.options.debug || this.options.verbose) {
          console.log(`DAntiSpam (log#ChannelNotFound): The mod log channel was not found.`);
        }
      }
    }
  }
   ...
   */
  async banUser(message, member, lang, spamMessages) {
    ...
    if (!member.bannable) {
      ...
      await this.sendActionMessage(message, "ban", lang);
      if (this.options.modLogsEnabled) {
   ...
   */
  async muteUser(message, member, lang, spamMessages) {
    if (this.options.removeMessages && spamMessages) {
      this.clearSpamMessages(spamMessages, message.client);
    }
    this.cache.messages = this.cache.messages.filter((u) => u.authorID !== message.author.id);
    const userCanBeMuted =
      message.guild.members.me.permissions.has(Discord.PermissionFlagsBits.ModerateMembers) &&
      message.guild.members.me.roles.highest.position > message.member.roles.highest.position &&
    ...
    await this.sendActionMessage(message, "mute", lang);
    if (this.options.modLogsEnabled) {
   ...
   */
  async kickUser(message, member, lang, spamMessages) {
    if (this.options.removeMessages && spamMessages) {
      this.clearSpamMessages(spamMessages, message.client);
    }
    this.cache.messages = this.cache.messages.filter((u) => u.authorID !== message.author.id);
    ...
    if (!member.kickable) {
      ...
      this.sendActionMessage(message, "kick", lang);
      if (this.options.modLogsEnabled) {
   ...
   */
  async warnUser(message, member, lang, spamMessages) {
    if (this.options.removeMessages && spamMessages) {
      this.clearSpamMessages(spamMessages, message.client);
    }
    this.cache.warnedUsers.push(message.author.id);
    this.log(message, `warned`, message.client);
    this.sendActionMessage(message, "warn", lang);
   ...
   * });
   */
  async message(message, lang, settings) {
    const { options } = this;
    ...
    const isChannelIgnored =
      typeof ignoredChannels === "function"
        ? ignoredChannels(message.channel)
    ...
    const memberHasIgnoredRoles =
      typeof ignoredRoles === "function"
        ? ignoredRoles(member.roles.cache)
        : ignoredRoles.some((r) => member.roles.cache.has(r));
    ...
    /*if (
      ...
      return false;*/
    let warnEnabled = settings.show_warning;
    ...
    const userCanBeBanned =
      banEnabled && !this.cache.bannedUsers.includes(message.author.id) && !sanctioned;
    if (userCanBeBanned && spamMatches.length >= options.banThreshold) {
      this.banUser(message, member, lang, spamMatches);
      sanctioned = true;
    } else if (userCanBeBanned && duplicateMatches.length >= options.maxDuplicatesBan) {
      this.banUser(message, member, lang, [
        ...duplicateMatches, ...spamOtherDuplicates,
      ]);
      sanctioned = true;
    }

    const userCanBeMuted = muteEnabled && !sanctioned;
    if (userCanBeMuted && spamMatches.length >= options.muteThreshold) {
      this.muteUser(message, member, lang, spamMatches);
      sanctioned = true;
    } else if (userCanBeMuted && duplicateMatches.length >= options.maxDuplicatesMute) {
      this.muteUser(message, member, lang, [
        ...duplicateMatches, ...spamOtherDuplicates,
      ]);
      sanctioned = true;
    }
    ...
    const userCanBeKicked =
      kickEnabled && !this.cache.kickedUsers.includes(message.author.id) && !sanctioned;
    if (userCanBeKicked && spamMatches.length >= options.kickThreshold) {
      this.kickUser(message, member, lang, spamMatches);
      sanctioned = true;
    } else if (userCanBeKicked && duplicateMatches.length >= options.maxDuplicatesKick) {
      this.kickUser(message, member, lang, [
        ...duplicateMatches, ...spamOtherDuplicates,
      ]);
      sanctioned = true;
    }
    ...
    const userCanBeWarned =
      warnEnabled && !this.cache.warnedUsers.includes(message.author.id) && !sanctioned;
    if (userCanBeWarned && spamMatches.length >= options.warnThreshold) {
      this.warnUser(message, member, lang, spamMatches);
      sanctioned = true;
    } else if (userCanBeWarned && duplicateMatches.length >= options.maxDuplicatesWarn) {
      this.warnUser(message, member, lang, [
        ...duplicateMatches, ...spamOtherDuplicates,
      ]);
      sanctioned = true;
    }
