...
class DiscordMusicBot extends Client {
  ...
  build() {
    ...
    /*const hook = new webhook.Webhook(
      "https://discord.com/api/webhooks/1138514329558327316/FrLo_6pxRlQeM3fjvS0pJ44cxaNsvpUq6ni3HFq46AxX44TEAfUQjMl94uGW3jCabJGp",
    );*/
    ...
    this.worddb_writer = fs.createWriteStream(`./info/word_db.txt`, {
      flags: "a",
    });
    ...
    this.strongword_filter = content_filter.preprocessWordLists(
      strong_words,
      whitelist,
      {
        /*considerPrecedingApostrophes: false,
    ...
    this.manager
      ...
      .on("SegmentSkipped", (player, queue, payload) => {
        this.log(
          `${colors.blue(client.guilds.cache.get(player.options.guildId).name)}(${
            ...
            payload.segment.category
          } segment has been skipped in a Youtube video. ${this.ms(
            payload.segment.start,
          )}->${this.ms(payload.segment.end)}`,
        );
      })
      .on("nodeConnect", (node) =>
        this.warn(
      ...
      .on("trackStart", async (player, track, payload) => {
        ...
        let trackStartedEmbed = this.Embed()
          ...
          .setDescription(
            track.info.uri && title
              ? `[${title}](${track.info.uri})`
              : lang.no_descriptions,
          )
          .addFields(
            {
          ...
          if (autoQueue) {
            ...
            res.tracks.some((track, index) => {
              ...
              for (let a = 0; a < player.queue.previous.length; a++) {
                if (
                  player.queue.previous[a].info.identifier ===
                ...
                .send({
                  embeds: [
                    new EmbedBuilder()
                      ...
                      .setDescription(
                        `${lang.could_not_load_track} ${res.exception.message}`,
                      ),
                  ],
                })
                .then((msg) => {
  ...
  async ServiceChannels() {
      ...
      if (err) {
        ...
        for (const file of files) {
          ...
          const settings_db = new JsonDB(
            new Config(EventsDir + "/" + file, true, true, "/"),
          );
          ...
          if (await settings_db.exists("/word_game")) {
            this.wordgame_status.push(
              decode(
                fs.readFileSync(`./info/wordgame/${guild.id}_wordgame.json`),
              ),
            );
          }
    ...
    schedule.scheduleJob("send_namedays", "0 3 * * *", async () => {
      ...
      if (!apiError) {
        const message1 =
          messageText1 + names1.map((name) => `**${name}**`).join(", ");
        const message2 =
          messageText2 + names2.map((name) => `**${name}**`).join(", ");
          ...
          if (channel) {
            this.postThrottle.add(() => {
              return channel.send(`${message1}\n${message2}`).catch((e) => {
                this.error(
                  `Failed to send the nameday message to ${channel.guild.id}`,
                );
              });
            });
          }
    ...
    schedule.scheduleJob(`update_word_cache`, "10 */5 * * * *", async () => {
      if (this.last_cache_size !== this.wordgame_cache.length) {
        for (
          let i = this.last_cache_size;
          i < this.wordgame_cache.length;
          i++
        ) {
  ...
  setup_counter_channel(guild) {
    schedule.scheduleJob(
      ...
      async () => {
        let guildSettings = this.guild_settings.find(
        ...
        if (
          onlineMembers !==
          +guildSettings.settings_db.data.counter_channel
            .counter_channel_prev_value
        ) {
          guildSettings.settings_db.data.counter_channel.counter_channel_prev_value =
            onlineMembers;
          await guild.channels.cache
            .get(
              guildSettings.settings_db.data.counter_channel.counter_channel_id,
            )
            .setName(
  ...
  setup_gamestat_channel(guild) {
    schedule.scheduleJob(
      ...
      async () => {
        ...
        const targetGame =
          guildSettings.settings_db.data.gamestat_channel.gamestat_channel_game
            .toLowerCase()
            .trim();
        ...
        if (
          playing_users !==
          +guildSettings.settings_db.data.gamestat_channel
            .gamestat_channel_prev_value
        ) {
      ...
      async () => {
        ...
        if (moment(target_datetime).isAfter(moment(), "minute")) {
          if (difference.asMinutes() >= 1440) {
            timer_data = difference.format(
              `d[${lang.short_days}] H[${lang.short_hours}]`,
            );
            timer_data =
            ...
            timer_data = difference.format(
              `H[${lang.short_hours}] m[${lang.short_minutes}]`,
            );
            timer_data =
              guildSettings.settings_db.data.countdown_channel.countdown_channel_scheme.replace(
                "{i}",
          ...
          await guild.channels.cache
            .get(
              guildSettings.settings_db.data.countdown_channel
                .countdown_channel_id,
            )
            .setName(
              guildSettings.settings_db.data.countdown_channel
                .countdown_channel_end_text,
            );
  ...
  async setup_tempchannel(tempChannel_temp, guild) {
    ...
    if (tempChannel_temp.temp_channel_scheme === "Number") {
      this.tempChannelsmanager.registerChannel(
        tempChannel_temp.temp_channel_id,
        {
          ...
          childVoiceFormatRegex: new RegExp(
            "/^[1-9][0-9]?" + lang.temp_channel_numbered + "/",
          ),
          childMaxUsers: 25,
          childBitrate: 96000,
    ...
    if (tempChannel_temp.temp_channel_scheme === "Name") {
      this.tempChannelsmanager.registerChannel(
        tempChannel_temp.temp_channel_id,
        {
          ...
          childAutoDeleteIfOwnerLeaves: false,
          childVoiceFormat: (str, count) =>
            `${str}${lang.temp_channel_usernane}`,
          childVoiceFormatRegex: new RegExp(
            "/^.*" + lang.temp_user_nane + "/",
  ...
  async poll_add_vote(reaction, user) {
    ...
    const poll_settings_pos = this.polls
      .map((e) => e.messageId)
      .indexOf(reaction.message.id);

    if (poll_settings_pos !== -1) {
      let pool_settings =
        await this.polls[poll_settings_pos].poll_data.getData("/");
  ...
  async load_polls() {
    ...
    fs.readdir(EventsDir, async (err, files) => {
      if (err) {
        ...
        for (const file of files) {
          const poll_db = new JsonDB(
            new Config(EventsDir + "/" + file, false, true, "/"),
          );
  ...
  async end_pool(channelId, messageId) {
    if (await this.checkIfMessageExists(channelId, messageId)) {
      let channel = this.channels.cache.get(channelId);
      const old_message = await channel.messages
        .fetch(messageId)
        .catch(() => undefined);
      const poll_settings_pos = this.polls.findIndex(
        (e) => e.messageId === old_message.id,
      );
      let result =
        await this.polls[poll_settings_pos].poll_data.getData("/emojis");
      ...
      let embed = new EmbedBuilder()
        .setTitle(
          `${lang.poll_title} ${await this.polls[
            poll_settings_pos
          ].poll_data.getData("/title")}`,
        )
        .setColor(
          await this.polls[poll_settings_pos].poll_data.getData("/embedColor"),
        )
      ...
      if (await this.polls[poll_settings_pos].poll_data.exists("/image")) {
        embed.setThumbnail(
          await this.polls[poll_settings_pos].poll_data.getData("/image"),
        );
      }
      await channel.send({
        embeds: [embed], //!
  ...
  sort(obj, valSelector) {
    const sortedEntries = Object.entries(obj).sort((a, b) =>
      valSelector(a[1]) < valSelector(b[1])
        ? 1
        : valSelector(a[1]) > valSelector(b[1])
          ? -1
          : 0,
  ...
  is_it_shortened(message) {
    return this.url_shorteners.some((shortener) =>
      message.toLowerCase().includes(shortener),
    );
  }
  listLinkShorterners() {
    try {
      const linkShortenersFile = fs.readFileSync(
        "./info/link-shorteners.txt",
        "utf-8",
      );
      const linkShortenersSet = new Set(
        linkShortenersFile.split("\n").map((line) => line.trim()),
      );
  ...
  setup_feeds() {
    schedule.scheduleJob(`feed_updater`, "30 */15 * * * *", async () => {
      ...
      for (let i = 0; i < this.feed_list.length; i++) {
        ...
        let send_feed_here = this.channels.cache.get(
          this.feed_list[i].channel_id,
        );
        if (!send_feed_here) {
          continue;
        }
        ...
        try {
          ...
          for (const item of this.twitch_list) {
            ...
            const stream = twitch_response.data.find(
              (data) =>
                data.user_name.toLowerCase() === item.twitch_user.toLowerCase(),
            );
            if (stream) {
              if (item.last_status === "offline") {
                ...
                if (["@everyone", "@here"].includes(item.mention)) {
                ...
                const embed = new EmbedBuilder()
                  ...
                  .setDescription(
                    `:link: https://www.twitch.tv/${stream.user_name}`,
                  )
  ...
  setup_youtube() {
    schedule.scheduleJob(`youtube_updater`, "30 */20 * * * *", async () => {
      ...
      for (let i = 0; i < this.youtube_list.length; i++) {
        ...
        let message_text = "";
        let send_feed_here = this.channels.cache.get(
          this.youtube_list[i].channel_id,
        );
        if (!send_feed_here) {
          continue;
        }
        ...
        for (let a = 0; a < youtube_data.length; a++) {
          ...
          if (
            moment(TimeToCheck).isAfter(
              moment(this.youtube_list[i].last_posted_date),
            )
          ) {
            const guildSettings = this.guild_settings.find(
              (e) => e.guildId === send_feed_here.guild.id,
            ...
            if (["@everyone", "@here"].includes(this.youtube_list[i].mention)) {
