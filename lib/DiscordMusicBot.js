...
class DiscordMusicBot extends Client {
  ...
  setup_openspy_channel(guild) {
    schedule.scheduleJob(
      `${guild.id}_openspy`,
      `${this.randomIntFromInterval(20, 55)} */5 * * * *`,
      async () => {
        let guildSettings = this.guild_settings.find((e) => e.guildId === guild.id);
        ...
        try {
          ...
          totalPlayers = servers.reduce((sum, server) => sum + (server.numplayers || 0), 0);
          activeServers = servers.filter((server) => server.numplayers > 0).length;
        } catch (error) {
          console.error("Player count error:", error);
        }
        await guild.channels.cache
          .get(guildSettings.settings_db.data.openspy_channel.openspy_channel_id)
          .setName(
            guildSettings.settings_db.data.openspy_channel.openspy_channel_scheme.replace(
              "{n}",
              totalPlayers,
            ),
          );
