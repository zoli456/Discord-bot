...
module.exports.randomcolor = async () => {
  return await fetch("https://api.popcat.xyz/randomcolor").then((r) =>
    r.json(),
  );
};
...
module.exports.imdb = async function (name) {
  ...
  const res = await fetch(
    `https://api.popcat.xyz/imdb?q=${encodeURIComponent(name)}`,
  ).then((r) => r.json());
  if (res.error) throw new Error(res.error);
  return res;
};
module.exports.steam = async function (name) {
  ...
  const res = await fetch(
    `https://api.popcat.xyz/steam?q=${encodeURIComponent(name)}`,
  );
...
module.exports.screenshot = async function (url) {
  ...
  const { isurl } = await fetch(
    `https://api.popcat.xyz/isurl?url=${encodeURIComponent(url)}`,
  ).then((r) => r.json());
  if (isurl === false)
...
module.exports.subreddit = async function (subreddit) {
  ...
  const res = await fetch(
    `https://api.popcat.xyz/subreddit/${encodeURIComponent(subreddit)}`,
  );
  const text = await res.text();
  return JSON.parse(text);
};
...
module.exports.github = async function (username) {
  ...
  const res = await fetch(
    `https://api.popcat.xyz/github/${encodeURIComponent(username)}`,
  );
  const js = await res.text();
  const obj = JSON.parse(js);
  if (obj.error)
    throw new Error(
      "[Popcat Wrapper] Invalid username in the github function!",
    );
  return obj;
};
module.exports.weather = async function (place) {
  ...
  const res = await fetch(
    `https://api.popcat.xyz/weather?q=${encodeURIComponent(place)}`,
  );
  const js = await res.text();
  const obj = JSON.parse(obj);
  if (obj.error)
    throw new Error("[Popcat Wrapper] Invalid place in the weather function!");
  return obj;
};
...
module.exports.instagramUser = async function (username) {
  ...
  if (account.includes("error"))
    throw new Error("[Popcat Wrapper] Not a valid instagram user!");
...
module.exports.reverse = async function (text) {
  ...
  const res = await fetch(
    `https://api.popcat.xyz/reverse?text=${encodeURIComponent(text)}`,
  );
  const json = await res.json();
  return json.text;
};
...
module.exports.colorinfo = async function (color) {
  ...
  const res = await fetch(
    `https://api.popcat.xyz/color/${encodeURIComponent(colour)}`,
  );
...
module.exports.itunes = async function (x) {
  ...
  const res = await fetch(
    `https://api.popcat.xyz/itunes?q=${encodeURIComponent(x)}`,
  );
  const json = await res.text();
  if (json.includes("error"))
    throw new Error("[Popcat Wrapper] Song Not found on iTunes!");
  return JSON.parse(json);
};
