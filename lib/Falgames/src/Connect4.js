...
import { EmbedBuilder, MessageFlags, ActionRowBuilder } from "discord.js";
import approve from "../utils/approve.js";

export default class Connect4 extends approve {
  /**
  ...
  constructor(options = {}) {
    ...
    if (!options.requestMessage)
      options.requestMessage = "{player} has invited you for a round of **Connect4**.";
    if (!options.rejectMessage)
      options.rejectMessage = "The player denied your request for a round of **Connect4**.";
    ...
    if (options.playerOnlyMessage !== false) {
      if (!options.playerOnlyMessage)
        options.playerOnlyMessage = "Only {player} and {opponent} can use these buttons.";
  ...
  async handleButtons(msg) {
    ...
    collector.on("collect", async (btn) => {
      ...
      if (block.y === 0) {
        ...
        if (column > 3)
          components[column % 4] = ButtonBuilder.from(components[column % 4]).setDisabled(true);
  ...
  getTurnMessage(msg) {
    return this.formatTurnMessage(this.options, msg ?? "turnMessage").replace(
      "{emoji}",
      this.getPlayerEmoji(),
    );
  }
