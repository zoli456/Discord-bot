...
export default class FastType extends events {
  /**
  ...
  constructor(options = {}) {
    ...
    if (!options.embed.description)
      options.embed.description = "You have {time} seconds to type the sentence below.";
    ...
    if (!options.loseMessage)
      options.loseMessage = "You lost! You didn't type the correct sentence in time.";
  ...
  async startGame() {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        this.options.embed.description.replace("{time}", this.options.timeoutTime / 1000),
      )
  ...
  gameOver(msg, result, timeRanOut) {
    ...
    GameOverMessage = GameOverMessage.replace("{time}", Math.floor(this.timeTaken / 1000)).replace(
      "{wpm}",
      this.wpm,
    );
