...
export default class FishyGame extends events {
  /**
  ...
  constructor(options = {}) {
    ...
    if (!options.player.id)
      options.player.id = options.message[options.isSlashGame ? "user" : "author"].id;
    ...
    if (!options.catchMessage)
      options.catchMessage = "You caught a {fish}. You paid {amount} for the fishing rod.";
    if (!options.sellMessage)
      options.sellMessage = "You sold {amount}x {fish} {type} items for a total of {price}.";
    if (!options.noBalanceMessage)
      options.noBalanceMessage = "You don't have enough balance to rent a fishing rod.";
    ...
    if (!options.invalidAmountMessage)
      options.invalidAmountMessage = "Amount must be between 0 and fish max amount.";
    if (!options.noItemMessage)
      options.noItemMessage = "You don't have any of this item in your inventory.";
    ...
    if (!options.message.deferred && options.isSlashGame)
      options.message.deferReply().catch((e) => {});
  ...
  async fishyInventory() {
    const fishes = [
      ...
      .map(
        (e) =>
          `**\u2000${this.fishes[e.toLowerCase()].emoji} ${e} Fish** — ${this.player.fishes[e.toLowerCase()] || 0}`,
      )
      .join("\n\n");
