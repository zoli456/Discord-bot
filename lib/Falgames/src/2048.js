import { EmbedBuilder, ActionRowBuilder, AttachmentBuilder, MessageFlags } from "discord.js";
import {
  disableButtons,
  formatMessage,
  move,
...
export default class TwoZeroFourEight extends events {
  /**
  ...
  constructor(options = {}) {
    ...
    if (typeof options.scoreText !== "string")
      throw new TypeError("INVALID_TEXT: scoreText option must be a string.");
  ...
  getBoardImage() {
    ...
    for (let y = 0; y < this.length; y++) {
      for (let x = 0; x < this.length; x++) {
        ...
        ctx.fillRect(
          x * (squareSize + gap) + gap,
          y * (squareSize + gap) + gap,
          squareSize,
          squareSize,
        );
  ...
  async gameOver(msg, result) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setImage("attachment://gameboard.png")
      .addFields({
        name: this.options.totalScoreText,
