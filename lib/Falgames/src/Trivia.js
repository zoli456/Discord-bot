...
module.exports = class Trivia extends events {
  ...
  constructor(options = {}) {
    ...
    if (!options.winMessage)
      options.winMessage = "You won! The correct answer is {answer}.";
    if (!options.loseMessage)
      options.loseMessage = "You lost! The correct answer is {answer}.";
  ...
  async startGame() {
    ...
    collector.on("collect", async (btn) => {
      ...
      return this.gameOver(
        msg,
        this.trivia.options[this.selected - 1] === this.trivia.answer,
      );
    });
      ...
      if (this.selected) {
        if (this.trivia.answer !== this.trivia.options[this.selected - 1])
          row.components[this.selected - 1].setStyle(
            this.options.falseButtonStyle,
          );
        else
          row.components[this.selected - 1].setStyle(
            this.options.trueButtonStyle,
          );
      }
    } else {
