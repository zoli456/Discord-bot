...
module.exports = class Podium {
  async getImage(image1, image2, image3, name1, name2, name3) {
    ...
    if (!isValid2)
      return console.error(`You must provide a valid image URL or buffer.`);
    if (!image3)
      return console.error(`You must provide an image as a third argument.`);
    let isValid3 = await validateURL(image3);
    if (!isValid3)
      return console.error(`You must provide a valid image URL or buffer.`);
    if (!name1)
      return console.error(`You must provide a text as a fourth argument.`);
    if (!name2)
      return console.error(`You must provide a text as a fifth argument.`);
    if (!name3)
      return console.error(`You must provide a text as a sixth argument.`);
    const canvas = await createCanvas(1173, 686);
    const ctx = canvas.getContext(`2d`);
    image1 = await loadImage(image1);
    image2 = await loadImage(image2);
    image3 = await loadImage(image3);
    const background = await loadImage(`${__dirname}/../../assets/podium.png`);
